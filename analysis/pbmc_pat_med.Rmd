...
ScRNA seq data from Calini representing PBMCs from 2 individuals (male age 60,52) in different storing conditions:
- fresh
- fixed with MetOH
- stored in 15% DMSO
...
##Add patient id
sce$patient <- ifelse(grepl("D301089", sce$info), "pat1", "pat2")
sce$media <- ifelse(grepl("Fresh", sce$info), "fresh", ifelse(grepl("DMSO", sce$info), "DMSO", "MetOH"))
...
nbcells <- cbind(table(sce$sample_id),table(sce$sample_id[!sce$isOutlier]))
...
#remove sample with bad qc
sce <- sce[,!sce$sample_id %in% "CR057"]
```
...
DimPlot(seurat, reduction = "tsne", group.by = "sample_id")
...
# Save data
saveRDS(sce, file = paste0(out_path, "/sce_pbmc_pat_med.rds"))
```
